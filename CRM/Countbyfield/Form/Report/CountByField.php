...
class CRM_Countbyfield_Form_Report_CountByField extends CRM_Report_Form {
  ...
  public function preProcess() {
    // want to create group_by here for all the things..xxx
    parent::preProcess();
  }
      ...
      if (array_key_exists('fields', $table)) {
        foreach ($table['fields'] as $fieldName => $field) {
          if (!empty($field['required']) ||
            ...
            if ($fieldName == 'counted') {
              $select[] = "COUNT(DISTINCT {$table['alias']}.id) as {$alias}";
            }
            else {
   ...
   * Key info we need is:
   *
   * The user input is in a form as:
   *     $this->_params['order_bys']['1']['column'] = (string [9]) `custom_10`
   *
   * The columns are described like
   *     $this->_columns[$tableName]['fields'][$fieldName]['dbAlias']
   */
  public function groupBy() {
    // Convert Sort By to order by.
    foreach ($this->_params['order_bys'] as $order) {
      // Find this column.
      $col = null;
      foreach ($this->_columns as $tableName => $table) {
        if (array_key_exists($order['column'], $table['fields'])) {
          $group_bys []= $table['fields'][$order['column']]['dbAlias'];
        }
      }
    }
    if ($group_bys) {
  ...
  public function orderBy() {
    $count_field = 'civicrm_contact_counted';
    $this->_orderBy = "ORDER BY $count_field DESC";
  }
